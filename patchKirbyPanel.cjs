...
const path = (fs.existsSync('vendor/getkirby') ? 'vendor/getkirby/cms' : 'kirby') + '/panel/dist/js/app.js'
const build = fs.readFileSync(path, 'utf8')
const pattern = '},this.marks)}'
const patch = ',...(\'marksResolver\' in window.panel.plugins?window.panel.plugins.marksResolver():{})' + pattern
